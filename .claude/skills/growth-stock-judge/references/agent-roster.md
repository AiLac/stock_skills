...
7 个并行研究子代理 + 1 个综合代理。每个研究子代理只负责一个模块、独立联网取证、按 `assets/agent-findings-schema.json` 回传，并只填**自己拥有的因子/惩罚/红线键**（其余留 `null`/`false`）。证据分级沿用 serenity 的 `references/evidence-ladder.md`；不在场时用 SKILL.md 内精简分级表兜底。各键定义以 `scripts/growth_scorecard.py` 为准。
