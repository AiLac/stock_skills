...
- 结构性风险：竞争颠覆、客户集中度、监管/地缘、会计治理红旗、宏观/流动性。
...
- **"待核实"是最后手段，不是偷懒出口**：凡是有明确源路径、能联网查到的（10-Q/10-K/8-K 里的 SBC 净稀释、客户集中度、出口管制最新细节、下代产品时间线、Q+1 共识是否已消化指引等），**先去查、查到就写进上文对应章节**；只有真正取不到的（实时期权 IV、未来才发生的事件、非公开数据）才留在"待核实"并写明查证路径。报告第 10 节的"待核实"清单应当**很短**。
