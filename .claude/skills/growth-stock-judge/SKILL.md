...
4. **(B) 的换路清单**（取证失败必试，试过再说"拿不到"）：
   - SEC 文件：主文档 `sec.gov/Archives/...htm` 原始页 → EDGAR 全文检索（efts.sec.gov / EDGAR full-text search）→ 文件镜像（bamsec / last10k / stocktitan / `data.sec.gov` API 带正常 UA）→ 该文件的 exhibits → 公司 IR 同稿。
   - 现价 / 估值倍数：换多个行情·数据源交叉。
...
1. 结论框（一句话结论 + 信心 + 双轴 + 总分）2. 评分卡（因子表 + **惩罚项表【每项含「理由」列】** + 红线表）3. 投资逻辑 4. 增长引擎+市场空间 5. 📊 最近财报重点 6. 护城河/产业链位置 7. 单位经济性/盈利路径 8. 估值与买点 9. ⚠️ 风险与利空（结构性 → 资金面 → 期权市场怎么看 → 下行情景 → kill-switch 清单）10. 结论复述+下一步要核实+催化/日历 11. 证据与来源。
