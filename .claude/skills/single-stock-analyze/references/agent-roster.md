...
- **Mission**：把"这是一家什么公司 + 核心产品 + 竞争格局"讲**详细且结构化**，写报告第 3 节「公司介绍 / 核心产品」，并为其余代理提供共同上下文。详见 `references/company-profile.md`。
- **拥有的模块 / 流程步骤**：锁定公司（流程 1）+ 报告第 3 节「公司介绍 / 核心产品」。
- **取证重点（八块）**：① 一句话画像+商业模式 ② 业务分部与收入拆分(带占比%) ③ 核心产品/服务逐个(是什么/卖给谁/变现/收入占比/生命周期) ④ 客户与集中度 ⑤ 竞品分析(对手/定位/市占/交锋点) ⑥ 竞争力分析(相对强弱，事实层) ⑦ 产业链位置 ⑧ 公司沿革与规模。
- **与 moat-economics-analyst 分工**：本代理出**描述性**竞品/竞争力事实；壁垒**能否持续**的判断归 moat-economics（报告第 7 节「护城河 / 产业链位置」）。竞品/竞争力事实喂给它，两节交叉引用不重复。
- **回传字段**：仍**仅 context**（不拥有任何因子/惩罚/红线键，`suggested_*` 全留 `null`/`false`）；产出结构化 `findings[]` 与 `summary` 供综合代理写第 3 节「🏢 公司介绍 / 核心产品」、供其他代理引用。
- **证据要求**：10-K/10-Q 业务描述、公司官网/IR、招股书为强证据；市占/竞争拆解可用行业报告/卖方(中证据)。每条标 强/中/弱/待核实。

## earnings-analyst

- **Mission**：深读最近一次财报，拿最新鲜最硬的证据（见 `references/earnings-readthrough.md`）。
- **拥有的模块 / 流程步骤**：最近财报深读（流程 2，第 6 节「📊 最近财报重点」）。
...
- **拥有的模块 / 流程步骤**：风险与利空（流程 8，第 11 节「⚠️ 风险与利空」）。
...
- **拥有的模块 / 流程步骤**：资金面 + 期权（流程 8，第 11 节「⚠️ 风险与利空」）。
...
- **预期再定价合成（出报告前必做）**：把 earnings-analyst 的「财报后反应 + 远期一致预期修正方向」、valuation-analyst 的「隐含预期 / 背离」、positioning-options-analyst 的「价格行为」**合成为一个**「市场在边际上重估什么 / 基本面 vs 股价背离是否合理」读数，**强制写进第 10 节「估值与买点」的「市场在重估什么 / 基本面 vs 股价背离」子块**，并据此校准（连同解释）`entry_expectations`。护栏：价格属中/弱证据，解释背离、不按涨跌机械打分；存在背离时第 1 节「结论」one-liner 须点名。
- **回传字段**：不新拥有键——**调和全部因子/惩罚/红线键**，以 `assets/scorecard-input.json` 为模板复制临时文件填分（勿覆盖模板）后运行 `scripts/growth_scorecard.py`（含双轴、惩罚、红线封顶），按 `assets/stock-verdict-template.md` 写"结论先行"的 13 节报告。
